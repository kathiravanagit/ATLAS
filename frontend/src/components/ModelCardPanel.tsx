    ...
    <div className="card p-5">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-base font-semibold text-white">ML Model Card</h3>
      ...
      >
        <div className="mt-4 space-y-4">
          ...
          <div className="bg-[#1c1917] rounded-xl p-3 border border-[#78350f]/40 flex items-start gap-2">
            <AlertTriangle size={14} className="text-[#f59e0b] mt-0.5 shrink-0" />
            <div className="text-[11px] text-[#d4d4d8] leading-relaxed">
              <span className="text-[#f59e0b] font-medium">Class imbalance: </span>
              Fraud rate is {((card.positive_ratio) * 100).toFixed(1)}% (1 in {imbalanceRatio}). Accuracy is inflated by the majority class — <span className="text-white font-medium">Recall ({card.recall}%) and F1 ({card.f1_score}%) are the primary metrics</span>.
            </div>
          </div>
          ...
          <div className="bg-[#0a0a0f] rounded-xl p-4 border border-[#27272a]">
            <div className="flex items-center justify-between mb-3">
              <div className="text-[10px] text-[#d4d4d8] uppercase tracking-wider">Performance (Fraud Class)</div>
              <div className="text-[10px] text-[#d4d4d8]">{card.ensemble_method}</div>
            </div>
            <div className="grid grid-cols-4 gap-4">
              {[
                ...
                >
                  ...
                  <div className="text-[11px] text-white font-medium mt-1">{m.label}</div>
                  <div className="text-[9px] text-[#71717a] mt-0.5">{m.desc}</div>
                </motion.div>
              ))}
            </div>
          </div>
          ...
          {card.confusion_matrix && (
            <div className="bg-[#0a0a0f] rounded-xl p-4 border border-[#27272a]">
              <div className="text-[10px] text-[#d4d4d8] uppercase tracking-wider mb-3">Confusion Matrix (40k test set)</div>
              <div className="grid grid-cols-3 gap-1 text-center" style={{ maxWidth: 280 }}>
                <div />
                <div className="text-[9px] text-[#71717a]">Pred+Fraud</div>
                <div className="text-[9px] text-[#71717a]">Pred+Normal</div>
                <div className="text-[9px] text-[#71717a] text-right pr-2">Actual+Fraud</div>
                <div className="bg-[#166534]/30 rounded py-1.5 text-[11px] text-[#22c55e] font-mono">{card.confusion_matrix.tp}</div>
                <div className="bg-[#991b1b]/30 rounded py-1.5 text-[11px] text-[#ef4444] font-mono">{card.confusion_matrix.fn}</div>
                <div className="text-[9px] text-[#71717a] text-right pr-2">Actual+Normal</div>
                <div className="bg-[#991b1b]/30 rounded py-1.5 text-[11px] text-[#ef4444] font-mono">{card.confusion_matrix.fp}</div>
                <div className="bg-[#166534]/30 rounded py-1.5 text-[11px] text-[#22c55e] font-mono">{card.confusion_matrix.tn.toLocaleString()}</div>
              </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {modelColors.map((m, i) => (
              <motion.div
                ...
                className="bg-[#0a0a0f] rounded-lg p-3 border border-[#27272a]"
              >
                ...
                <div className="flex items-end gap-2">
                  ...
                  <div className="text-[9px] text-[#71717a] mb-1">accuracy</div>
                </div>
          ...
          <div className="grid grid-cols-4 gap-2">
            {[
              ...
              <div key={i} className="bg-[#0a0a0f] rounded-lg p-2.5 border border-[#27272a] text-center">
                <div className="flex items-center justify-center gap-1 mb-1">{item.icon}<span className="text-[9px] text-[#d4d4d8] uppercase">{item.label}</span></div>
                <div className="text-[11px] text-white font-medium">{item.value}</div>
              </div>
            ))}
          </div>
          ...
          <div>
            <div className="text-[10px] text-[#d4d4d8] uppercase tracking-wider mb-2">Top Features by Importance</div>
              ...
              {card.feature_columns.slice(0, 8).map((feat, i) => {
                ...
                return (
                  <div key={feat} className="flex items-center gap-2">
                    <div className="w-[130px] text-[10px] text-[#e4e4e7] text-right truncate">{feat.replace(/_/g, ' ')}</div>
                    <div className="flex-1 h-3 bg-[#0a0a0f] rounded overflow-hidden border border-[#27272a]">
                      <motion.div
                        initial={{ width: 0 }}
                    ...
                    <div className="w-[60px] text-[9px] text-[#d4d4d8] font-mono text-right">
