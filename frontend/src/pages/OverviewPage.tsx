  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-white">Cash-Out Risk Intelligence</h2>
          <p className="text-base text-[#e4e4e7] mt-1">ATLAS — Proactive cybercrime intervention across 8 Indian cities</p>
        </div>
      ...
      <div className="grid grid-cols-6 gap-4">
        <StatCard icon={FolderOpen} label="Active Cases" value={stats.active_cases} sub="Under investigation" color="default" />
        <StatCard icon={MapPin} label="High-Risk Locations" value={stats.high_risk_locations} sub="Predicted" color="error" />
        <StatCard icon={Bell} label="Alerts Today" value={stats.alerts_today + liveAlertCount} sub="Pending review" color="warning" />
        <StatCard icon={Clock} label="Avg. Lead Time" value={stats.avg_lead_time} sub="Before expected window" color="success" />
        <StatCard icon={ShieldCheck} label="Estimated Exposure" value={`₹${(stats.prevented_fraud / 100000).toFixed(1)}L`} sub="Resolved case value" color="success" />
        <StatCard icon={AlertOctagon} label="Mules Flagged" value={stats.mules_flagged} sub="High-risk accounts" color="error" />
      </div>
      <div className="text-[9px] text-[#52525b] text-right">Source: Synthetic data (200k transactions, 8 cities, 400 ATMs) | Fraud rate: 3.65% | Model: RF+XGBoost ensemble</div>
