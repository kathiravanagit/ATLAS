  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4">
        <button
          onClick={() => navigate(-1)}
          className="p-2 rounded-lg bg-[#18181b] border border-[#27272a] hover:border-[#71717a] transition-colors"
        >
          <ArrowLeft size={16} className="text-[#d4d4d8]" />
        </button>
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Profile Settings</h1>
          <p className="text-sm text-[#d4d4d8] mt-1">Manage your account details and security credentials</p>
        </div>
      </div>
      ...
      <div className="bg-[#18181b] border border-[#27272a] rounded-xl overflow-hidden">
        ...
        <div className="p-6 space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              <label className="block text-[10px] text-[#d4d4d8] uppercase tracking-wider mb-1.5">Full Name</label>
              <input
                type="text"
                value={name}
                onChange={e => setName(e.target.value)}
                className="w-full px-3 py-2 bg-[#0a0a0f] border border-[#27272a] rounded-lg text-sm text-white focus:outline-none focus:border-[#3b82f6] transition-colors"
              />
            </div>
            <div>
              <label className="block text-[10px] text-[#d4d4d8] uppercase tracking-wider mb-1.5">Email Address</label>
              <input
                type="email"
                value={email}
                disabled
                className="w-full px-3 py-2 bg-[#0a0a0f] border border-[#27272a] rounded-lg text-sm text-[#71717a] cursor-not-allowed"
              />
              <p className="text-[10px] text-[#71717a] mt-1">Email cannot be changed</p>
            </div>
            <div>
              <label className="block text-[10px] text-[#d4d4d8] uppercase tracking-wider mb-1.5">Badge ID</label>
              <div className="relative">
                <Hash size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#71717a]" />
                <input
                  type="text"
                  value={badge}
                  onChange={e => setBadge(e.target.value)}
                  className="w-full pl-8 pr-3 py-2 bg-[#0a0a0f] border border-[#27272a] rounded-lg text-sm text-white focus:outline-none focus:border-[#3b82f6] transition-colors"
                  placeholder="IPB-2026-XXXX"
                />
              </div>
            </div>
            <div>
              <label className="block text-[10px] text-[#d4d4d8] uppercase tracking-wider mb-1.5">Department</label>
              <div className="relative">
                <Building2 size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#71717a]" />
                <input
                  type="text"
                  value={department}
          ...
          {/* Read-only fields */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 pt-3 border-t border-[#27272a]">
            <div>
              <label className="block text-[10px] text-[#d4d4d8] uppercase tracking-wider mb-1.5">Role</label>
              <div className="px-3 py-2 bg-[#0a0a0f] border border-[#27272a] rounded-lg text-sm text-[#3b82f6] font-mono">
                {ROLE_LABELS[role] || role}
              </div>
            </div>
            <div>
              <label className="block text-[10px] text-[#d4d4d8] uppercase tracking-wider mb-1.5">Account Created</label>
              <div className="px-3 py-2 bg-[#0a0a0f] border border-[#27272a] rounded-lg text-sm text-[#71717a]">
                {createdAt ? new Date(createdAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }) : '—'}
              </div>
            </div>
            <div>
              <label className="block text-[10px] text-[#d4d4d8] uppercase tracking-wider mb-1.5">Last Login</label>
              <div className="px-3 py-2 bg-[#0a0a0f] border border-[#27272a] rounded-lg text-sm text-[#71717a]">
      ...
      <div className="bg-[#18181b] border border-[#27272a] rounded-xl overflow-hidden">
        ...
        <div className="p-6 space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <div>
              <label className="block text-[10px] text-[#d4d4d8] uppercase tracking-wider mb-1.5">Current Password</label>
              <div className="relative">
                <input
                  type={showCurrent ? 'text' : 'password'}
                  value={currentPassword}
                  onChange={e => setCurrentPassword(e.target.value)}
                  className="w-full px-3 pr-10 py-2 bg-[#0a0a0f] border border-[#27272a] rounded-lg text-sm text-white focus:outline-none focus:border-[#3b82f6] transition-colors"
                  placeholder="Enter current password"
                />
                <button
                  type="button"
                  onClick={() => setShowCurrent(!showCurrent)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-[#71717a] hover:text-[#d4d4d8]"
                >
            ...
            <div>
              <label className="block text-[10px] text-[#d4d4d8] uppercase tracking-wider mb-1.5">New Password</label>
              <div className="relative">
                <input
            ...
            <div>
              <label className="block text-[10px] text-[#d4d4d8] uppercase tracking-wider mb-1.5">Confirm New Password</label>
              <input
                type="password"
          ...
          <div className="flex justify-end">
            <button
              ...
              disabled={changingPw}
              className="flex items-center gap-2 px-5 py-2 bg-[#f59e0b] hover:bg-[#d97706] text-black text-sm font-medium rounded-lg transition-colors disabled:opacity-50"
            >
              <Lock size={14} />
      ...
      {/* Footer */}
      <div className="text-center text-[10px] text-[#71717a] pb-8">
